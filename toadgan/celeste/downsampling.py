...
def celeste_downsampling(
    ...
    interp_args={"mode": "bilinear", "align_corners": False},
):
    """
    ...
    for sc in range(num_scales):
        ...
        tmp = bil_scaled[0]
        zeros = th.zeros(
            (tmp.shape[-2], tmp.shape[-1]), dtype=th.int64, device=image.device
        )
        ...
        before_softmax = (
            th.nn.functional.one_hot(zeros, LevelRenderer.max_idx + 1)
            .transpose(0, 2)
            .transpose(1, 2)[None, :, :, :]
        )
