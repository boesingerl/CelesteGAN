...
def format_and_use_generator(curr_img, G_z, count, Z_opt, pad_noise, pad_image, noise_amp, G, opt, scaler):
...
def draw_concat(generators, noise_maps, reals, noise_amplitudes, pad_noise, pad_image, opt, scaler, start_img=None, in_s=None, replace_scale=1):
    ...
    for count, (G, Z_opt, real_curr, noise_amp) in enumerate(
        ...
        G_z = format_and_use_generator(z, G_z, count, Z_opt,
                                       pad_noise, pad_image, noise_amp, G, opt, scaler)
...
class LevelGen:
    ...
    def to_json(self,path):
        with gzip.open(path, 'wb') as handle:
            gen_dict = {'gen_bytes': [base64.b64encode(v.getvalue()).decode() for v in self.bytes]}
            gen_dict['noise_maps'] = noise_maps = [np.asarray(nmap).tolist() for nmap in self.noise_maps]
            gen_dict['noise_amps']  = [np.asarray(amp).tolist() for amp in self.noise_amplitudes]
    ...
    def read_json(path):
        ...
        read_gen = [ort.InferenceSession(base64.b64decode(allbytes), providers=['CPUExecutionProvider']) for allbytes in load_dict['gen_bytes']]
...
def force_path(sample):
    ...
    yfin, xfin = [np.sort(x)[x.shape[0]//2] for x in np.where(sample[0].argmax(0) == 32)]
    ysta, xsta = [np.sort(x)[x.shape[0]//2] for x in np.where(sample[0].argmax(0) == 20)]

    path = nx.astar_path(G, (xfin, yfin), (xsta, ysta), heuristic=dist, weight="weight")

    PROBLEMS = ['bounceBlock', 'bigSpinner', 'coverupWall', 'crushBlock', 'dashBlock', 'fallingBlock', 'fireBall', 'moveBlock', 'seeker', 'sinkingPlatform', 'spikesDown', 'spikesLeft', 'spikesRight', 'spikesUp', 'spinner', 'zipMover']
